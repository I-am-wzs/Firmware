...
dname=$(dirname "$PWD")
fastrtps_dir=$dname/toolschain/eProsima_FastRTPS-1.5.0-Linux
...
then
    ...
    (cd eProsima_FastCDR-1.0.7-Linux && ./configure --libdir=/usr/lib && make -j4 && sudo make install)
    (cd eProsima_FastRTPS-1.5.0-Linux && ./configure --libdir=/usr/lib && make -j4 && sudo make install)
    rm -rf requiredcomponents 
    popd
fi
